...
public class Crawler extends Thread {
	...
	private Extract_Links EL = new Extract_Links();
        //前端管理器：用来管理两个多级 URL链接队列：站点单级优先队列和站内多级优先队列。其中，站
        //点单级优先队列按优先级存放着欲访问的网站首页的 URL，而站内多级优先队列存放着欲访问的某
        //个网站的按优优级排序的一级、二级及三级链接
	...
	public Crawler(int domain, String langid) {
		...
		try {
			...
			if (first_crawling_or_not == true) {
				...
				rf.seek(0);                 //找到文件指针0处  即最开始 标上false
				rf.writeBytes("false");   
				rf.close();
